...
| <nobr>`ant_ai.topology`</nobr> | [`Ensemble`][ant_ai.topology.runtime.Ensemble] | Adaptive topology. Recomputes which agents can reach which between rounds: a strategy is an ordered list of [`Stage`][ant_ai.topology.plan.Stage]s, each transforming the next round's plan, a [`TopologyMaterialiser`][ant_ai.topology.materialise.TopologyMaterialiser] rebinds peer tools or routes messages, and an [`InteractionGraph`][ant_ai.topology.graph.InteractionGraph] records both the reachability granted and the calls actually made. Published methods live in `ant_ai.topology.builtins`. Declared on the Colony via `topology()`; see [Topology architecture](topology.md) for the design and [Adaptive topology](../multi-agent/topology.md) for usage. |
...
flowchart TD
    ...
    subgraph hm["ant_ai"]
        ...
        subgraph hooks_sub["ant_ai.hooks"]
            ...
            PIIHook["PIIGuardrailHook"]
            LLMGuardrail["LLMGuardrailHook"]
        end
...
| [`TopologyEvent`][ant_ai.core.events.TopologyEvent]   | `"topology"`  | An adaptive topology was decided for a round; `links` holds the directed edges with their scores and reasons |
| `HealingEvent`                                        | `"healing"`   | A detector found a structural failure in collaboration; holds the pattern, the detector and the corrections applied (importable from `ant_ai.core.events`) |
...
| [`ToolResultEvent`][ant_ai.core.events.ToolResultEvent]                   | `"tool_result"`       | One tool call completed; `content` holds the result, `name` and `tool_call_id` identify the originating call   |
